...
import { getDailyHoroscope, getRemedyAdvice, generateHoroscopeVideo, checkGlobalVideoUrl, StructuredHoroscope, getYearlyForecast, YearlyForecast, injectBatchData, verifyConnection, fetchGlobalBlogPosts, GLOBAL_STORAGE_BASE_URL, getVastuDetails } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  // VASTU & FENG SHUI
  const [vastuType, setVastuType] = useState<'Vastu' | 'Feng Shui'>('Vastu');
  const [vastuRoom, setVastuRoom] = useState('Bedroom');
  ...
  const handleVastu = async () => { setVastuLoading(true); setVastuResult(await getVastuDetails(vastuType, vastuRoom)); setVastuLoading(false); };
  ...
  const navItems = [
    ...
    { id: AppSection.VASTU, icon: 'fa-yin-yang', label: 'Vastu & Feng Shui', visible: true },
  ...
  return (
    <div className="min-h-screen vedic-gradient flex flex-col md:flex-row relative">
      ...
      <main className="flex-1 p-4 md:p-8 lg:p-12 h-screen overflow-y-auto scroll-smooth relative pb-24 md:pb-8">
        ...
        {activeSection === AppSection.DASHBOARD && (
            <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div onClick={() => handleNavClick(AppSection.VASTU)} className="glass-gold p-6 rounded-2xl cursor-pointer hover:bg-[#d97706]/10">
                        <i className="fas fa-yin-yang text-2xl text-teal-400 mb-2"></i>
                        <h3 className="font-bold text-white">Vastu & Feng Shui</h3>
                    </div>
                    {isPremium && (
        ...
        {/* VASTU & FENG SHUI */}
        {activeSection === AppSection.VASTU && (
            <div className="max-w-2xl mx-auto glass p-8 rounded-3xl border border-[#d97706]/30">
                <h3 className="text-2xl font-cinzel text-[#d97706] mb-6 text-center">Energy Balancing</h3>
                <div className="grid grid-cols-2 gap-4 mb-4">
                    <button onClick={() => setVastuType('Vastu')} className={`py-2 rounded-lg font-bold border transition-all ${vastuType === 'Vastu' ? 'bg-[#d97706] text-white border-[#d97706]' : 'border-white/20 text-stone-500'}`}>Vastu Shastra</button>
                    <button onClick={() => setVastuType('Feng Shui')} className={`py-2 rounded-lg font-bold border transition-all ${vastuType === 'Feng Shui' ? 'bg-[#d97706] text-white border-[#d97706]' : 'border-white/20 text-stone-500'}`}>Feng Shui</button>
                </div>
                <div className="mb-4">
                    <label className="text-xs text-stone-500 uppercase">Select Area</label>
                    <select value={vastuRoom} onChange={(e) => setVastuRoom(e.target.value)} className="w-full bg-black/40 border border-[#d97706]/30 rounded-lg p-3 text-white mt-1 outline-none">
                        <option value="Main Entrance">Main Entrance</option>
                        <option value="Bedroom">Bedroom</option>
                        <option value="Kitchen">Kitchen</option>
                        <option value="Home Office">Home Office/Study</option>
                        <option value="Living Room">Living Room</option>
                        <option value="Bathroom">Bathroom</option>
                    </select>
                </div>
                <button onClick={handleVastu} disabled={vastuLoading} className="w-full bg-white/10 text-white font-bold py-3 rounded-lg hover:bg-white/20 transition-all border border-white/10">
                    {vastuLoading ? 'Analyzing Energy...' : 'Get Tips'}
                </button>
                {vastuResult && (
                    <div className="mt-6 glass bg-teal-900/10 p-6 rounded-xl border border-teal-500/20 animate-fade-in">
                        <h4 className="text-teal-400 font-bold mb-3"><i className="fas fa-wind mr-2"></i> Cosmic Advice</h4>
                        <div className="text-stone-300 font-serif leading-relaxed whitespace-pre-wrap">{vastuResult}</div>
                    </div>
                )}
            </div>
        )}
